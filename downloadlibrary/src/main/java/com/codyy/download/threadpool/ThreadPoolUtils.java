...
public class ThreadPoolUtils {
     ...
     */
    public ThreadPoolUtils(@ThreadPoolType int type, int corePoolSize) {
        // 构造有定时功能的线程池
        ...
        switch (type) {
            case ThreadPoolType.FIXED_THREAD:
                // 构造一个固定线程数目的线程池
                ...
                break;
            case ThreadPoolType.SINGLE_THREAD:
                ...
                break;
            case ThreadPoolType.CACHED_THREAD:
                // 构造一个缓冲功能的线程池
                // ThreadPoolExecutor(0, Integer.MAX_VALUE, 60L, TimeUnit.SECONDS, new SynchronousQueue<Runnable>());
                exec = Executors.newCachedThreadPool();
                break;
            case ThreadPoolType.DEFAULT_THREAD:
                exec = scheduleExec;
                break;
        }
    }
     ...
     * @param command 命令
     */
    public <T extends Runnable> void execute(T command) {
     ...
     * @param commands 命令链表
     */
    public <T extends Runnable> void execute(List<T> commands) {
